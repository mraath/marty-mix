DECLARE @mobileUnitId     BIGINT = -351340188657890982;
...
--Peripheral Devices
  ...
  LEFT JOIN [library].[Devices] ld WITH (NOLOCK) ON ld.DeviceKey = dmdlpd.PeripheralDeviceKey AND ld.LibraryKey = tcg.LibraryKey -- This is in @GeneralConfigGroupInfo
...
FROM @AllLinesByMobileUnitId a --It is in @AllConfigGroupLines
  INNER JOIN @EffectiveByMobileUnitId eff ON eff.[MobileUnitId] = a.[MobileUnitId]

CREATE OR ALTER PROCEDURE [template].[Template_GetConfigurationGroupsOtherColumns]
  @groupId BIGINT
AS
BEGIN
  ...
  FROM @GeneralConfigGroupInfo g
    ...
    -- Join to the child device's definition to access its SystemName
    INNER JOIN [definition].[Devices] logical_fw_device WITH (NOLOCK)
        ON logical_fw_device.DeviceKey = dep.ChildDeviceKey
    -- Join the template's instance of that child device
    INNER JOIN [template].[Devices] td WITH (NOLOCK)
        ON td.MobileDeviceTemplateKey = g.MobileDeviceTemplateKey
        AND td.DeviceKey = logical_fw_device.DeviceKey 
    -- Now that we have the correct logical device, get its properties, filtering by the logical device's name

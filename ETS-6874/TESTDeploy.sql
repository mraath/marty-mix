...
--DROP PROCEDURE [template].[Template_GetConfigurationGroupsOtherColumns]
...
SELECT DISTINCT
  ...
  [FWName] = fw.Name
FROM @GeneralConfigGroupInfo g
  -- Join template.Devices first to filter early
  INNER JOIN [template].[Devices] td WITH (NOLOCK)
    ON td.MobileDeviceTemplateKey = g.MobileDeviceTemplateKey
    AND td.LibraryKey = g.LibraryKey
  -- Get firmware properties early with the FW version filter
  INNER JOIN [template].[DeviceProperties] tdpr WITH (NOLOCK)
    ...
    AND tdpr.Value IS NOT NULL  -- Filter out NULLs early
  -- Now check if this device is a dependency (reduces records to check)
  ...
  -- Finally get the firmware version details

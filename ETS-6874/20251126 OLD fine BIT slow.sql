...
BEGIN
  ...
  WITH GeneralConfigGroupInfo AS (
    ...
    SELECT
      ConfigurationGroupId,
      -- Concatenate C1 and C2 values with comma separator
      STUFF((
        SELECT ', ' + Connection
        FROM AllConfigGroupLines l2
        WHERE l2.ConfigurationGroupId = l.ConfigurationGroupId
          AND l2.WireName IN ('C1', 'C2')
          AND l2.Connection IS NOT NULL
        FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 2, '') AS CanScript,
      STUFF((
        SELECT ', ' + CAST(LineId AS NVARCHAR(50))  -- CAST LineId to string
        FROM AllConfigGroupLines l2
        WHERE l2.ConfigurationGroupId = l.ConfigurationGroupId
          AND l2.WireName IN ('C1', 'C2')
          AND l2.LineId IS NOT NULL
        FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 2, '') AS CanScriptLineId,

...
-- Constants CTE for better performance and maintainability
WITH Constants AS (
    SELECT 
        ...
        FIRMWARE_VERSION = CAST(642299852142387816 AS BIGINT),
        PreferedFirmwareVersionPropId = CAST(4015466679217121645 AS BIGINT),
        FM3xBASDDR = 2,
        FMCANDDMs = 3,
        FM3XXX_MOBILE_DEVICE_RANGE = CAST(6773205951411395052 AS BIGINT),
        MSG_SENDCONFIG = 254,
        MSG_SENDFIRMWARE = 103,
        MSG_SENDSETTINGS = 255
),
PropertyKeys AS (
    SELECT 
        PropIMEIKey = (SELECT TOP 1 PropertyKey FROM [DeviceConfiguration].[definition].[Properties] WITH (NOLOCK) WHERE PropertyId = (SELECT UNIT_IMEI FROM Constants)),
        PropStreamaxKey = (SELECT TOP 1 PropertyKey FROM [DeviceConfiguration].[definition].[Properties] WITH (NOLOCK) WHERE PropertyId = (SELECT StreamaxSerialNumber FROM Constants)),
        FWVersionPropKey = (SELECT TOP 1 PropertyKey FROM [DeviceConfiguration].[definition].[Properties] WITH (NOLOCK) WHERE PropertyId = (SELECT PreferedFirmwareVersionPropId FROM Constants))
)
...
CROSS JOIN PropertyKeys pk
LEFT JOIN [DeviceConfiguration].[mobileunit].[MobileUnitProperties] mup WITH (NOLOCK)
    ON mu.[MobileUnitKey] = mup.[MobileUnitKey] AND mup.[PropertyKey] = pk.PropIMEIKey
LEFT JOIN [DeviceConfiguration.DataProcessing].[state].[MobileUnitState] mus WITH (NOLOCK)
    ON mus.[MobileUnitId] = mu.[MobileUnitId] AND mus.[PropertyId] = (SELECT SERIAL_NUMBER FROM Constants)
LEFT JOIN [DeviceConfiguration.DataProcessing].[state].[MobileUnitState] mus2 WITH (NOLOCK)
    ON mus2.[MobileUnitId] = mu.[MobileUnitId] AND mus2.[PropertyId] = (SELECT StreamaxSerialNumber FROM Constants)
LEFT JOIN [DeviceConfiguration].[mobileunit].[MobileUnitProperties] mup2 WITH (NOLOCK)
    ON mu.[MobileUnitKey] = mup2.[MobileUnitKey] AND mup2.[PropertyKey] = pk.PropStreamaxKey;
...
WITH InstalledFW AS (
    ...
    WHERE mus.[PropertyId] = (SELECT FIRMWARE_VERSION FROM Constants)
)
UPDATE ur
...
WITH TemplateFW AS (
    ...
    INNER JOIN [DeviceConfiguration].[template].[DeviceProperties] tdpr WITH (NOLOCK)
        ...
        AND tdpr.PropertyKey = (SELECT TOP 1 PropertyKey FROM [DeviceConfiguration].[definition].[Properties] WITH (NOLOCK) WHERE PropertyId = (SELECT PreferedFirmwareVersionPropId FROM Constants))
...
WITH FilterFlags AS (
    SELECT 
        ...
        IsFMBasDevice = CASE WHEN EXISTS (
            ...
            WHERE ddd.ChildDeviceKey = ur.MobileDeviceKey AND ddParent.DeviceId = (SELECT FM3XXX_MOBILE_DEVICE_RANGE FROM Constants)
        ) THEN 1 ELSE 0 END,
        -- Check if CAN Incompatible
        IsCanBasIncompatible = CASE WHEN ur.FirmwareType = (SELECT FMCANDDMs FROM Constants) AND EXISTS (
...
FilteredVersions AS (
    ...
    WHERE 
        NOT (ff.IsCanBasIncompatible = 1 AND ff.FirmwareType = (SELECT FMCANDDMs FROM Constants) AND dfw.Name LIKE 'E%')
        AND NOT (ff.FirmwareType = (SELECT FM3xBASDDR FROM Constants) AND ff.IsFMBasDevice = 0)
),
VersionRanked AS (
    SELECT
        fv.MobileUnitId,
...
RankedMessages AS (
    ...
    WHERE mum.MessageSubType IN ((SELECT MSG_SENDCONFIG FROM Constants), (SELECT MSG_SENDFIRMWARE FROM Constants), (SELECT MSG_SENDSETTINGS FROM Constants))
),
...
MessageAlerts AS (
    SELECT
        ...
        IsConfigOrSettingAlert = MAX(CASE
                                        WHEN lms.MessageSubType IN ((SELECT MSG_SENDCONFIG FROM Constants), (SELECT MSG_SENDSETTINGS FROM Constants)) 
                                            AND lms.CreationDateUtc < DATEADD(day, -5, GETUTCDATE()) 
                                            AND kgs.StatusId IS NULL
                                        THEN 1 ELSE 0 END),
        IsFirmwareAlert = MAX(CASE
                                WHEN lms.MessageSubType = (SELECT MSG_SENDFIRMWARE FROM Constants)

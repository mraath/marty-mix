CREATE FUNCTION [state].[MobileUnit_GetUnitFirmwareInfoTVF_WithAlert5]
(
    @MobileUnitId BIGINT,
    @MobileUnitKey INT,
    @MobileDeviceKey INT,
...
(
	...
	InstalledFirmwareAlpha NVARCHAR(15),
	PreferredFirmwareAlpha NVARCHAR(15)
)
AS
BEGIN
	...
	DECLARE @InstalledFirmwareAlpha NVARCHAR(15) = (CASE WHEN NULLIF(@InstalledFirmwareName,'') IS NULL THEN '' ELSE LEFT(STUFF((SELECT RIGHT(REPLICATE('0',3) + [value],3) FROM STRING_SPLIT(@InstalledFirmwareName, '.') FOR XML PATH('')), 1, 0, ''), 15) END);
	DECLARE @PreferredFirmwareAlpha NVARCHAR(15) = (CASE WHEN NULLIF(@PreferredFirmwareName,'') IS NULL THEN '' ELSE LEFT(STUFF((SELECT RIGHT(REPLICATE('0',3) + [value],3) FROM STRING_SPLIT(@PreferredFirmwareName, '.') FOR XML PATH('')), 1, 0, ''), 15) END);

    INSERT INTO @FirmwareInfo (InstalledFirmwareName, PreferredFirmwareName, IsFirmwareOutdated, InstalledFirmwareAlpha, PreferredFirmwareAlpha) 
	VALUES (ISNULL(@InstalledFirmwareName, ''), ISNULL(@PreferredFirmwareName, ''), ISNULL(@IsFirmwareOutdated, 0), @InstalledFirmwareAlpha, @PreferredFirmwareAlpha);
    RETURN;
END;
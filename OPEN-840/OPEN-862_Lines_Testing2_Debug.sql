...
-- =================================================================
-- Step 1: Declare and populate your configuration group IDs
-- =================================================================
-- This section just sets up your data.
-- You can run this block on its own. It should complete without errors.
...
-- =================================================================
-- Step 2: Find the problematic IDs
-- =================================================================
-- The error "Error converting data type nvarchar to bigint" happens when SQL
-- tries to convert a text value to a number and fails.
--
-- Run this query to see if any of your IDs are not valid numbers.
-- If this returns any rows, those are your problem values.
-- It could be an empty string, a non-numeric character, or a value out of range.

SELECT id AS ProblematicId
FROM @configGroupIds
...
-- =================================================================
-- This is the first part of your main query. The error happens here
-- because of the join between `tcg.ConfigurationGroupId` (BIGINT) and `cg.id` (NVARCHAR).
-- SQL tries to convert every `cg.id` to BIGINT, and when it fails, the query stops.

-- To fix this, we use TRY_CAST(cg.id AS BIGINT). This will return NULL if the
-- conversion fails, and the join will simply ignore that row instead of erroring.

-- You can run this CTE by itself to confirm it works with the fix.
...
-- Run this to see the output of just the first CTE
PRINT '--- Testing GeneralConfigGroupInfo CTE ---';
WITH GeneralConfigGroupInfo AS (
    ...
    INNER JOIN [template].[ConfigurationGroups] tcg WITH (NOLOCK) ON tcg.ConfigurationGroupId = TRY_CAST(cg.id AS BIGINT) -- FIX APPLIED HERE
...
SELECT TOP 10 * FROM GeneralConfigGroupInfo;
GO

-- =================================================================
-- Step 4: Run the rest of the query, section by section
-- =================================================================
-- Now you can run each subsequent CTE to see the data as it's transformed.

DECLARE @StreamaxSerialNumber BIGINT = -4477362625925416557;
DECLARE @STREAMAX_STANDALONE_DEVICE_KEY INT = (SELECT [DeviceKey] FROM [definition].[Devices] WITH (NOLOCK) WHERE DeviceId = -1064000195705392069);

PRINT '--- Testing MobileUnits CTE ---';
WITH GeneralConfigGroupInfo AS (
    SELECT
        tcg.ConfigurationGroupId,
        dd.DeviceKey,
...
-- ... You can continue this pattern for the other CTEs (AllLines, PivotedLines, etc.)

-- =================================================================
-- Final corrected script
-- =================================================================
PRINT '--- Running final corrected script ---';

...
When looking into the table above, we can see that within **Rio Tinto**, that the alerts and the asset lines sometimes would time out. It would remain pending. This confirms my above statements. The config groups Alerts was also quite high, but it shares the stored proc as previously mentioned. So if we fix up the assets alerts, this will also be resolved.
...
Lastly, we could also look into getting the *config groups list*. However, this also usually takes less than a second. So I would also not prioritize this.
...
We can do this for both the Config Groups Grid and the Assets List Grid.

Once we have enhanced the stored procs and we have also ensured that the lazy loading happens at the correct time, we could look into loading only a few config groups assets at a time. However, for now, I think we should just focus on the above-mentioned enhancements, before adding **batch** loading.
